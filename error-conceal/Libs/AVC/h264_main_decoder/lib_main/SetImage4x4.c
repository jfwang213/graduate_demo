	...
	for( h = 0; h < 16; h++){
		...
		if ( ref_cache_l0[index8] >= 0 ){
			if( ref_cache_l1[index8] < 0 ){
				...
				ptr_aio_tImage_Cb[0] = _mm_extract_epi16(Cb_res,0); 
				ptr_aio_tImage_Cb[PicWidthInPix >> 2] = _mm_extract_epi16(Cb_res,1); 
				ptr_aio_tImage_Cr[0] = _mm_extract_epi16(Cr_res,0); 
				ptr_aio_tImage_Cr[PicWidthInPix >> 2] = _mm_extract_epi16(Cr_res,1); 
