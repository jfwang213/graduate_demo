...
@param slice The aio_pstSlice structure contains all information used to decode the next aio_pstSlice.
...
void filter_mb_B( unsigned char image[], unsigned char image_Cb[], unsigned char image_Cr[], SLICE *slice, PPS *pps, 
				 unsigned char slice_table[],  short mv_cache_l0[][2], short mv_cache_l1[][2], 
				 short ref_cache_l0[], short ref_cache_l1[], int mb_xy, int PicWidthInMbs, 
				 ...
				 const int x, const int y, RESIDU *residu) 
{
    ...
    for( dir = 0; dir < 2; dir++ )
    {
		...
		if (slice -> disable_deblocking_filter_idc  == 2 && slice_table[mbm_xy] != slice_table[mb_xy]){
			start = 1;
		}
        ...
        for( edge = start; edge < 4; edge++ ) {
			...
			if(!dir){
				...
				HorizontalEdgeFilter(slice -> disable_deblocking_filter_idc, edge, PicWidthInMbs, qp, qPCb, qPCr, bS, pps -> alpha_offset, pps -> beta_offset, image, image_Cb, image_Cr);
			}else {
				int qPCb = (qP_tab[qp_mb_xy + pps -> chroma_qp_index_offset] + qP_tab[qp_mbn_xy + pps -> chroma_qp_index_offset] + 1) >> 1;
				int qPCr = (qP_tab[qp_mb_xy + pps -> second_chroma_qp_index_offset] + qP_tab[qp_mbn_xy + pps -> second_chroma_qp_index_offset] + 1) >> 1;
				VerticalEdgeFilter(slice -> disable_deblocking_filter_idc, edge, PicWidthInMbs, qp, qPCb, qPCr, bS, pps -> alpha_offset, pps -> beta_offset, image, image_Cb, image_Cr);
			}
		}
		mbm_xy = top_mb_xy;
    }
...
@param image_Cr Table of current frame.
*/
void filter_B( SPS *Sps, PPS *pps,SLICE *slice, LIST_MMO *Current_pic, unsigned char slice_table[], DATA Tab_Block[], 
			  ...
			  unsigned char aio_tImage[], unsigned char image_Cb[], unsigned char image_Cr[])
{
	...
	const int PicSizeInMbs = Sps -> PicSizeInMbs ;
    const int PicWiInMbs = Sps -> pic_width_in_mbs;
	const int PicWidthInMbs = Sps -> pic_width_in_mbs + 2;
   ...
   //Loop filter process applied on the whole image
	for ( iCurrMbAddr = 0; iCurrMbAddr < PicSizeInMbs ;iCurrMbAddr++) {
		...
		filter_mb_B(&aio_tImage [offset_L +  ((x + (PicWidthInMbs << 4) * y) << 4)], 
			...
			&image_Cr [offset_C + ((x + (PicWidthInMbs << 3) * y) <<3)], slice, pps, slice_table, 
