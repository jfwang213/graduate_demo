						   ...
						   int mbPartIdx, char *RefIdxLx, short *RefCacheLx, short MvCacheLx[][2])
{
	...
	for ( subMbPartIdx = 0 ; subMbPartIdx < sub_num_b_part[ sub_mb_type] ; subMbPartIdx++ ) 	{
		...
		mvL0[0] = read_se(Data, Position);
		mvL0[1] = read_se(Data, Position);
...
*/
int sub_mb_pred_B ( int *dct_allowed, const unsigned char *RESTRICT Data, int *Position, 
...
int GetCavlcBRefList(const unsigned char *ai_pcData, int *aio_piPosition, char *refLx,
					 int Mode, unsigned char NumRefLx, unsigned char *MbPartMode, short Pred_Lx)
{
	...
	if ( NumRefLx > 0) {
		if(MbPartMode [0] != Pred_Lx){
			refLx [0] = read_te(ai_pcData, aio_piPosition, NumRefLx);
		}else {
			refLx [0] = -1; //mbl check
		}
		if (Mode != 3 && MbPartMode [1] != Pred_Lx){
			refLx [1] = read_te(ai_pcData, aio_piPosition, NumRefLx);
		}else {
			refLx [1] = -1;
		}
#ifdef ERROR_DETECTION
...
*/
int mb_pred_B ( const unsigned char *ai_pcData, int *aio_piPosition, const SLICE *aio_pstSlice, DATA *aio_pstMacroblock, 
			   RESIDU *CurrResidu, short mv_cache_l0[][2], short mv_cache_l1[][2], short *ref_cache_l0, 
			   short *ref_cache_l1, short *mvl1_l0, short *mvl1_l1, short *refl1_l0, short *refl1_l1, 
			   int direct_8x8_inference_flag, int long_term, int slice_type, int is_svc)
{
	...
	if(GetCavlcBRefList(ai_pcData, aio_piPosition, RefIdxL0, CurrResidu -> Mode, 
		aio_pstSlice -> num_RefIdxL0_active_minus1, aio_pstMacroblock -> MbPartPredMode, Pred_L1)){
			return 1;
	}


	if(GetCavlcBRefList(ai_pcData, aio_piPosition, RefIdxL1, CurrResidu -> Mode, 
		aio_pstSlice -> num_RefIdxL1_active_minus1, aio_pstMacroblock -> MbPartPredMode, Pred_L0)){
			return 1;
	}
