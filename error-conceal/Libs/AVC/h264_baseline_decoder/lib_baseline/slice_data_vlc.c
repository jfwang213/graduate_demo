...
int MoreRbsp( const unsigned char *ai_pcData, const int *aio_piPosition, const int ai_piNalBytesInNalunit, 
			 int iCurrMbAddr, int PicSizeInMbs, int *ErrorDetected)
{
...
*/
int slice_data_I_cavlc(RESIDU *picture_residu, const unsigned char *ai_pcData,  const int ai_piNalBytesInNalunit, 
						const SPS *Sps, const PPS *Pps, DATA aio_tstBlock [], SLICE *Slice, 
						int *aio_piPosition,const VLC_TABLES *Vlc, unsigned char aio_pstSlice_table [], 
						const unsigned char aio_pstSliceGroupMap [], unsigned char *EndOfSlice) 
{
	...
	__declspec(align(64)) unsigned char aio_tiNon_zero_count_cache[48];
	...
	do 	{
		...
		ErrorDetection = macroblock_layer_I(Pps, &picture_residu[iCurrMbAddr], ai_pcData, aio_piPosition, Slice, aio_tstBlock, 
			Vlc, aio_tiNon_zero_count_cache, aio_pstSlice_table, iMb_x, iMb_y, &Last_Qp);

		NonZeroCountManagement(aio_tiNon_zero_count_cache, iMb_x, iMb_y, PicWidthInMbs, PicHeightInMbs, 
...
*/
int slice_data_P_cavlc(RESIDU *picture_residu, const unsigned char *Data, const int ai_piNalBytesInNalunit, 
						const SPS *Sps, const PPS *Pps, DATA *Block, SLICE *Slice, int *Position, 
						const VLC_TABLES *Vlc, unsigned char *Slice_table, const unsigned char *SliceGroupMap, 
						unsigned char *EndOfSlice , short *mvl0_io, short *refl0_io)
{
	...
	int Last_Qp = Slice -> slice_qp_delta;
	int mb_skip_run, ErrorDetection = 0;
	...
	do 
	{
		...
		if (mb_skip_run){
			for ( i = 0 ; i < mb_skip_run && iCurrMbAddr < PicSizeInMbs; i++ ) {
				...
				SkipP(Slice, Sps, picture_residu, Block, Slice_table, 
		...
		if ( moreDataFlag ) {		
			...
			fill_caches_motion_vector(Slice, Slice -> b_stride, Slice -> b8_stride, 
				ref_cache_l0, mv_cache_l0, Slice_table , &picture_residu[iCurrMbAddr], 
				&mvl0_io [(iMb_x << 3) + iMb_y * (Slice -> b_stride << 2)], 
			...
			ErrorDetection = macroblock_layer_P(Pps, &picture_residu[iCurrMbAddr], Data, 
				...
				Slice_table, iMb_x, iMb_y, &Last_Qp, mv_cache_l0, ref_cache_l0);
