...
int macroblock_layer_P (const PPS *Pps, RESIDU *picture_residu,  const unsigned char *ai_pcData,  int *aio_piPosition, 
						SLICE *aio_pstSlice, DATA aio_tstBlock[], const VLC_TABLES * Vlc, 
						unsigned char aio_tiNon_zero_count_cache [ ], unsigned char aio_pstSlice_table [ ], const short ai_iMb_x, 
						const short ai_iMb_y , int *last_QP, short mv_cache_l0[][2], short *ref_cache_l0)
{  
	...
	if (mb_type < 5)	{
		...
		return macroblock_P_partitionning(Pps, picture_residu, ai_pcData, aio_piPosition, aio_pstSlice, 
			aio_pstBlock, Vlc, aio_tiNon_zero_count_cache, aio_pstSlice_table, ai_iMb_x, ai_iMb_y, last_QP, 
		...
		return macroblock_I_partitionning(Pps, picture_residu, ai_pcData, aio_piPosition, aio_pstSlice, 
			aio_pstBlock, Vlc, aio_tiNon_zero_count_cache, aio_pstSlice_table, ai_iMb_x, ai_iMb_y, last_QP, iCurrMbAddr);
	}
}
