...
int sub_mb_pred_P(int *dct_allowed, const unsigned char *ai_pcData, int *aio_piPosition, 
				  const SLICE *aio_pstSlice, RESIDU * residu, short mv_cache_l0[][2], short ref_cache_l0[]);
int mb_pred_P ( const unsigned char *ai_pcData, int *aio_piPosition, const SLICE *aio_pstSlice, 
...
static __inline void GetCavlcSubMbType(const unsigned char *Data,  int *Position, RESIDU *CurrResidu ){
	...
	CurrResidu -> SubMbType [0] = read_ue(Data, Position);
	CurrResidu -> SubMbType [1] = read_ue(Data, Position);
	CurrResidu -> SubMbType [2] = read_ue(Data, Position);
	CurrResidu -> SubMbType [3] = read_ue(Data, Position);
}
...
static __inline int GetCavlcIPredMode(const unsigned char *Data,  int *Position, short *intra4x4_pred_mode_cache, int luma4x4BlkIdx)
{
	...
	{
		int rem_intra4x4_pred_mode = getNbits(Data, Position, 3);
