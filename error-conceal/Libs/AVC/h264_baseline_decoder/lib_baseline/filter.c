...
@param aio_pstSlice The aio_pstSlice structure contains all information used to decode the next aio_pstSlice.
...
void filter_mb_P( unsigned char image[], unsigned char image_Cb[], unsigned char image_Cr[], const  SLICE *aio_pstSlice, 
				 unsigned char aio_pstSlice_table[], short mv_cache_l0[][2], short ref_cache_l0[], int PicWidthInMbs, 
				 const PPS *pps, const int x, const int y, const RESIDU *residu)
{
	int dir, edge, i, mv_done, qp, mbm_type;
	int mbn_type, qp_mbn_xy, edges;
	const int mb_xy = x + y * aio_pstSlice -> mb_stride;
	...
	for( dir = 0; dir < 2; dir++ ){
		...
		if (aio_pstSlice -> disable_deblocking_filter_idc  == 2 && aio_pstSlice_table[mbm_xy] != aio_pstSlice_table[mb_xy]){
			start = 1;	
		}
		...
		for( edge = start; edge < edges; edge++ ) {
			...
			if(!dir){
				...
				HorizontalEdgeFilter( aio_pstSlice -> disable_deblocking_filter_idc, edge, PicWidthInMbs, qp, qPCb, qPCr, bS, pps -> alpha_offset, pps -> beta_offset, image, image_Cb, image_Cr);
			}else {
				int qPCb = (qP_tab[qp_mb_xy + pps -> chroma_qp_index_offset] + qP_tab[qp_mbn_xy + pps -> chroma_qp_index_offset] + 1) >> 1;
				int qPCr = (qP_tab[qp_mb_xy + pps -> second_chroma_qp_index_offset] + qP_tab[qp_mbn_xy + pps -> second_chroma_qp_index_offset] + 1) >> 1;
				VerticalEdgeFilter(aio_pstSlice -> disable_deblocking_filter_idc, edge, PicWidthInMbs, qp, qPCb, qPCr, bS, pps -> alpha_offset, pps -> beta_offset, image, image_Cb, image_Cr);
			}
		}
		mbm_xy = mb_xy - aio_pstSlice -> mb_stride;
	}
}
...
@param image_Cr Table of current frame.
*/
void filter_P(const  SPS *aipstsps, const PPS *aipstpps, const SLICE *Slice,  unsigned char aio_pstSlice_table[],
			  ...
			  unsigned char image[], unsigned char image_Cb[], unsigned char image_Cr[])
{
	...
	for ( ; iCurrMbAddr < PicSizeInMbs ;iCurrMbAddr++) 	{
		...
		filter_mb_P(Y, U, V, Slice, aio_pstSlice_table, mv_cache_l0, ref_cache_l0, PicWidthInMbs, aipstpps, x, y, residu);
