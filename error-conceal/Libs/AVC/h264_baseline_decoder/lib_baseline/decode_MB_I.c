...
void decode_I_Intra4x4_lum ( unsigned char *image, const short PicWidthInPix, const RESIDU *residu, const short W4x4_intra [6][16],const mb4x4_mode_pf *mb4x4_mode)
{
	...
	for ( i = 0 ; i < 16 ; i ++ ) {       
		short locx = LOCX(i);
		short locy = LOCY(i);
		int mode = residu -> Intra16x16DCLevel[i]; 
					  ...
					  const RESIDU *residu, const W_TABLES *quantif, const mbChroma_mode_pf * mbChroma_mode_pf) 
{
	...
	if ( residu -> Cbp & 0x30){
		...
		QPc = qP_tab [residu -> Qp + pps -> chroma_qp_index_offset];
...
void decode_MB_I (unsigned char *image, unsigned char *image_Cb, unsigned char *image_Cr, const PPS *pps, 
	...
	if ( residu -> MbType == INTRA_4x4 ) {
		...
		decode_I_Intra4x4_lum(image, PicWidthInPix << 4, residu, quantif -> W4x4_intra, Allpf_struct -> Ptr_mb4x4_mode);

		// Decode chrominances
		decode_I_chroma(image_Cb, image_Cr, PicWidthInPix << 3,  pps, residu, quantif, Allpf_struct -> Ptr_mbChroma_mode_pf);
	} 
	else if (residu -> MbType == INTRA_16x16 ) {   
		// Intra16x16 MB
		// Decode luminances
		decode_I_Intra16x16_lum(image, PicWidthInPix << 4, residu, quantif -> W4x4_intra, Allpf_struct -> Ptr_mb16x16_mode);
		...
		// Intra16x16 MB
		// Decode luminances
		decode_I_Intra8x8_lum(image, PicWidthInPix << 4, residu, quantif -> W8x8_intra);
		// Decode chrominances
		decode_I_chroma(image_Cb, image_Cr, PicWidthInPix << 3, pps, residu, quantif, Allpf_struct -> Ptr_mbChroma_mode_pf);
	...
	else if (residu -> MbType == INTRA_PCM )
	{
		...
		for (i = 0 ; i < 16; i++){
			for ( j = 0 ; j < 16; j++){
				image[(PicWidthInPix << 4)* i + j] =  (unsigned char)residu -> LumaLevel[(i << 4) + j];
			}
		}

		for (i = 0 ; i < 8; i++){
			for ( j = 0 ; j < 8; j++){
				image_Cb[(PicWidthInPix << 3)* i + j] =  (unsigned char)residu -> ChromaACLevel_Cb[(i << 3) + j];
				image_Cr[(PicWidthInPix << 3)* i + j] =  (unsigned char)residu -> ChromaACLevel_Cr[(i << 3) + j];
			}
		}
	}
}
