...
void IDR_picture( const SPS *Sps, const PPS * Pps, const SLICE *aio_pstSlice, const unsigned char aio_pstSlice_table [ ], 
				 ...
				 unsigned char aio_tucImage[ ], unsigned char aio_tucImage_Cb[ ], unsigned char  aio_tucImage_Cr[ ])
{
	...
	if ( Sps -> PicSizeInMbs){
		...
		short iCurrMbAddr = aio_pstSlice -> first_mb_in_slice ;
		...
		for ( ; iCurrMbAddr < PicSizeInMbs ;) {
			...
			decode_MB_I(&aio_tucImage[((iMb_x + (iMb_y * PicWidthInMbs << 4)) << 4)], 
				...
				&picture_residu[iCurrMbAddr], PicWidthInMbs,  quantif, ai_struct_pf);
