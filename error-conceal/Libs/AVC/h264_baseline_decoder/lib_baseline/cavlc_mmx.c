...
void residual_block_cavlc_16_MMX( short *   ao_piCoeffLevel, const int ai_iMaxNumCoeff, const unsigned char *ai_poData, int *   position
							 , const int ai_iN,	const VLC_TABLES *vlc, unsigned char aio_tiNon_zero_count_cache [  ])
{
    ...
    int tmp1 = pred_non_zero_count(n, aio_tiNon_zero_count_cache);
	...
	*position += tmp5 ;
	...
	aio_tiNon_zero_count_cache [tmp] = coeff_token_TotalCoeff; //((aio_tiNon_zero_count_cache [tmp] & (~flag2)) | ((coeff_token_TotalCoeff) & (flag2)) );
