...
*/
static __inline void CheckNeighbourBlock(int dir, int edge, int add, int block1, int block2, char bS[4]){
...
void external_edge(const int mb_type, const int mbn_type, const int mb_xy, const int mbn_xy, char bS[4], 
				   int dir, int edge,int slice_type, int spatial_scalability, RESIDU *residu, 
	...
	if ( spatial_scalability && ( IS_I_SVC(mb_type) || IS_I_SVC(mbn_type))){
		...
		if( IS_I_SVC(mb_type) || IS_I_SVC(mbn_type)){
			...
			for( i = 0; i < 4; i++ ) {
				...
				if(residu[mb_xy] . blk4x4_Res & BMask || residu[mbn_xy] . blk4x4_Res & BnMask) {
					bS[i] = 2;
				}else {
