...
void Upsample_residu(short first_mb_in_slice, const NAL *Nal,const  SPS *sps_id, 
					 ...
					 short *luma_out,short* cb_out,short* cr_out)
{
	...
	for( i = first_mb_in_slice; i < PicSizeInMbs;){
		if (current_residu[i] . ResidualPredictionFlag){
			short x_pos = iMb_x << 4;
			short y_pos = iMb_y << 4;
			LumaVerticalFilter(&y16[y_pos], &yk16[y_pos],  &xp16[x_pos], &yp16[y_pos], Nal, base_residu, &luma_tmp[x_pos], &luma_out[x_pos + y_pos * (Nal -> curr_Width + 32)]);
		}
		//Updating the macroblock address
		i = Next_MbAddress(i, PicSizeInMbs,PicWidthInMbs, ai_tiMbToSliceGroupMap, &iMb_x, &iMb_y);
	}
	...
	for( i = first_mb_in_slice; i < PicSizeInMbs;){
		...
		if (current_residu[i] . ResidualPredictionFlag){
			short x_pos = iMb_x << 3;
			short y_pos = iMb_y << 3;
			ChromaVerticalFilter(&y16[y_pos], &yk16[y_pos], &yp16[y_pos], Nal, &luma_tmp[x_pos], &cb_out[x_pos + y_pos * ((Nal -> curr_Width + 32) >> 1)]);
		}
	...
	for( i = first_mb_in_slice; i < PicSizeInMbs;){
		...
		if (current_residu[i] . ResidualPredictionFlag){
			short x_pos = iMb_x << 3;
			short y_pos = iMb_y << 3;
			ChromaVerticalFilter(&y16[y_pos], &yk16[y_pos], &yp16[y_pos], Nal, &luma_tmp[x_pos], &cr_out[x_pos + y_pos * ((Nal -> curr_Width + 32) >> 1)]);
