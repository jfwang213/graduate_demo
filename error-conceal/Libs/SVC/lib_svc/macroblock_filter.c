...
//For the upsampling
void filter_mb( unsigned char image[], unsigned char image_Cb[] , unsigned char image_Cr[], const PPS *pps, 
			   short mb_xy, short PicWidthInMbs, const int transform_8x8_size, int spatial_scalable_flg, 
			   int DeblockingFilterIdc,const short x, const short y, const RESIDU *residu) 
{
	...
	for( dir = 0; dir < 2; dir++ )
	{
		...
		for( edge = start; edge < 4; edge++ ) {
			...
			if(!dir){
				...
				HorizontalEdgeFilter(DeblockingFilterIdc, edge, PicWidthInMbs, qp, qPCb, qPCr, bS, pps -> alpha_offset_upsapmpling, pps -> beta_offset_upsapmpling, image, image_Cb, image_Cr);
			}else {
				int qPCb = (qP_tab[qp_mb_xy + pps -> chroma_qp_index_offset] + qP_tab[qp_mbn_xy + pps -> chroma_qp_index_offset] + 1) >> 1;
				int qPCr = (qP_tab[qp_mb_xy + pps -> second_chroma_qp_index_offset] + qP_tab[qp_mbn_xy + pps -> second_chroma_qp_index_offset] + 1) >> 1;
				VerticalEdgeFilter(DeblockingFilterIdc, edge, PicWidthInMbs, qp, qPCb, qPCr, bS, pps -> alpha_offset_upsapmpling, pps -> beta_offset_upsapmpling, image, image_Cb, image_Cr);
			}
		}
		mbm_xy = top_mb_xy;
	}
...
void filter_mb_svc( unsigned char image[], unsigned char image_Cb[] , unsigned char image_Cr[], SLICE *slice, PPS *pps, 
				   unsigned char slice_table[], short mv_cache_l0[][2], short mv_cache_l1[][2], short ref_cache_l0[], 
				   short ref_cache_l1[], short mb_xy, short PicWidthInMbs, short poc_l0[], short poc_l1[], 
				   const short x, const short y, RESIDU *residu, const NAL *Nal, short Pass)
{
	...
	for( dir = 0; dir < 2; dir++ ){
		...
		if ((slice -> disable_deblocking_filter_idc  == 2 || slice -> disable_deblocking_filter_idc == 5 ) && slice_table[mbm_xy] != slice_table[mb_xy]){
			start = 1;
		}

		if ((Pass == 1) && slice_table[mbm_xy] != slice_table[mb_xy]){
			start = 1;
		}
		...
		for( edge = start; edge < 4; edge++ ) {
			...
			if (edge > 0){
				...
				if( Pass == 2){
					...
					qp = qp_mbn_xy = qp_mb_xy;
					internal_edge(mb_type, bS, dir, edge, slice -> slice_type, Nal -> SpatialScalability, 
						&residu[mb_xy], mv_cache_l0, ref_cache_l0, poc_l0, mv_cache_l1, ref_cache_l1, poc_l1);
				}
			}else{
				if ((Pass == 2) && slice_table[mbm_xy] == slice_table[mb_xy]){
					bS[0] = bS[1] = bS[2] = bS[3] = 0;
				} else {
					qp_mbn_xy = residu[mbm_xy] . QpLf;
					qp = (qp_mb_xy + qp_mbn_xy + 1) >> 1;
					external_edge(mb_type, mbm_type, mb_xy, mbm_xy, bS, dir, edge, slice -> slice_type, Nal -> SpatialScalability, 
			...
			if(!dir){
				...
				HorizontalEdgeFilter(slice -> disable_deblocking_filter_idc, edge, PicWidthInMbs, qp, qPCb, qPCr, 
					bS, pps -> alpha_offset, pps -> beta_offset, image, image_Cb, image_Cr);
			}else {
				int qPCb = (qP_tab[qp_mb_xy + pps -> chroma_qp_index_offset] + qP_tab[qp_mbn_xy + pps -> chroma_qp_index_offset] + 1) >> 1;
				int qPCr = (qP_tab[qp_mb_xy + pps -> second_chroma_qp_index_offset] + qP_tab[qp_mbn_xy + pps -> second_chroma_qp_index_offset] + 1) >> 1;
