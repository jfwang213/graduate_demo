...
*/
void RescaleCurrentCoeff(RESIDU *CurrResidu, RESIDU *BaseResidu, const PPS *Pps, const W_TABLES *quantif){
	...
	if( CurrResidu -> Cbp & 0xf){
		...
		//When the cbp is equal to zero, we have to update the transform8x8x flag with 
					...
					, short *Residu_Y, short *Residu_Cb, short *Residu_Cr)
{
	...
	if (CurrResidu -> Cbp){
		if ( !CurrResidu -> Transform8x8){
			Intra4x4_lum(Residu_Y, CurrResidu, PicWidthInPix, quantif -> W4x4_inter);
		} else {
			Intra8x8_lum(Residu_Y, CurrResidu, PicWidthInPix, quantif -> W8x8_inter);
		}
