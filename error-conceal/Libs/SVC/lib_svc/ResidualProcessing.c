...
void base_layer_residual(RESIDU *residu, short Luma[], short Chroma_Cb[], short Chroma_Cr[], 
	...
	if ( !residu -> Transform8x8){
		base_Intra4x4_lum(Luma, residu, PicWidthInPix, quantif -> W4x4_inter);
	} else {
		base_Intra8x8_lum(Luma, residu, PicWidthInPix, quantif -> W8x8_inter);
	}

	//Decode the chrominance
	base_chroma(Chroma_Cb, Chroma_Cr, residu, Pps, PicWidthInPix >> 1, quantif);
}
	...
	if (CurrResidu -> ResidualPredictionFlag){
		...
		if (CurrResidu -> Cbp){
			if ( !CurrResidu -> Transform8x8){
				decode_P_Intra4x4_lum(Y, CurrResidu, PicWidthInPix, quantif -> W4x4_inter);
			} else {
				decode_P_Intra8x8_lum(Y, CurrResidu, PicWidthInPix, quantif -> W8x8_inter);
			}
			//Decode the chrominance
					...
					short *Residu_Luma, short *Residu_Cb, short *Residu_Cr)
{
	...
	if (CurrResidu -> ResidualPredictionFlag){
		...
		CurrResidu -> blk4x4_Res = CurrResidu -> blk4x4;
		if ( !CurrResidu -> Transform8x8){
			Intra4x4_lum(Residu_Luma, CurrResidu, PicWidthInPix, Quantif -> W4x4_inter);
		}else {
			Intra8x8_lum(Residu_Luma, CurrResidu, PicWidthInPix, Quantif -> W8x8_inter);
		}

		//Decode the chrominance
		decode_chroma(Residu_Cb, Residu_Cr, CurrResidu, Pps, PicWidthInPix >> 1, Quantif);
	}
}
					...
					short *Residu_Luma, short *Residu_Cb, short *Residu_Cr)
{
	...
	if(Nal -> PicToDisplay){
		...
		assert(CurrResidu -> ResidualPredictionFlag > 0);
