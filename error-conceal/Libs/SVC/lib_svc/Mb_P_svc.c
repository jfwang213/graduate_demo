			   ...
			   const STRUCT_PF *vectors, REFPART RefPartition[16])
{
	...
	if ( Nal -> PicToDisplay ){
		...
		sample_interpolation(image, image_Cb, image_Cr, mv_cache, ref_cache, x , y, PicWidthInPix, PicHeightInPix, RefPicListL0, vectors -> Ptr_interpol_4x4);			
	}		
}
				 ...
				 const STRUCT_PF *vectors, REFPART RefPartition[16])
{
	...
	if ( Nal -> PicToDisplay ){
		//Recovery of  the prediction
		SampleInterpolation8x8( image, image_Cb, image_Cr, mv_cache, ref_cache, x, y,  PicWidthInPix, PicHeightInPix, RefPicListL0, vectors -> Ptr_interpol_8x8);
	}
}
