	...
	for ( subMbPartIdx = 0 ; subMbPartIdx < 4 ; subMbPartIdx++ ) {
		...
		if ( MotionPredLx [mbPartIdx] == 1){
			...
			if (!Nal -> non_diadic) {
				...
				MvCacheLx [index][0] = MV_CAL(MvBaseLx[index_mv], Nal -> up_Width, Nal -> down_Width) + MvdLx[(mbPartIdx << 2) + subMbPartIdx][0]; 
				MvCacheLx [index][1] = MV_CAL(MvBaseLx[index_mv + 1], Nal -> up_Height, Nal -> down_Height) + MvdLx[(mbPartIdx << 2) + subMbPartIdx][1]; 
			}else{
			 ...
			 REFPART RefPartition[16], short MvCacheLx[][2], short *RefCacheLX, short *MvBaseLx, short *RefBaseLx)
{
	...
	for ( subMbPartIdx = 0 ; subMbPartIdx < 2 ; subMbPartIdx++ ) {
		...
		if ( MotionPredLx [mbPartIdx] == 1){
			...
			if ( !Nal -> non_diadic){
				int index_mv  = RefPartition [Idx] . MvIndex;
				mvL0 [0] = MV_CAL(MvBaseLx[index_mv], Nal -> up_Width, Nal -> down_Width) + MvdLx[( mbPartIdx << 2) + subMbPartIdx][0]; 
				mvL0 [1] = MV_CAL(MvBaseLx[index_mv + 1], Nal -> up_Height, Nal -> down_Height) + MvdLx[( mbPartIdx << 2) + subMbPartIdx][1]; 
			}else{
			 ...
			 REFPART *RefPartition, short MvCacheLx[][2], short *RefCacheLX, short *MvBaseLx, short *RefBaseLx)
{
	...
	if ( MotionPredLx [mbPartIdx] == 1){
		...
		if ( !Nal -> non_diadic){
			...
			mvL0[0] = MV_CAL(MvBaseLx [index_mv], Nal -> up_Width, Nal -> down_Width) + MvdLx[mbPartIdx << 2][0]; 
			mvL0[1] = MV_CAL(MvBaseLx [index_mv + 1], Nal -> up_Height, Nal -> down_Height) + MvdLx[mbPartIdx << 2][1]; 
		}else {
				 ...
				 short *mv_bas_l1, short *ref_bas_l0, short *ref_bas_l1, const STRUCT_PF *vectors, REFPART RefPartitionL0[16])
{
	...
	if ( aio_pBlock -> MbPartPredMode[0] == Pred_L0 || aio_pBlock -> MbPartPredMode[0] == B_Bi	){
		if ( aio_pBlock -> MotionPredL0 [0] == 1){
			...
			if ( !Nal -> non_diadic){
				...
				mvL0[0] = MV_CAL(mv_bas_l0[index_mv], Nal -> up_Width, Nal -> down_Width) + aio_pBlock -> MvdL0[0][0]; 
				mvL0[1] = MV_CAL(mv_bas_l0[index_mv + 1], Nal -> up_Height, Nal -> down_Height) + aio_pBlock -> MvdL0[0][1]; 
			}else{
	...
	if (aio_pBlock -> MbPartPredMode[0] == Pred_L1 ||	aio_pBlock -> MbPartPredMode[0] == B_Bi){
		if ( aio_pBlock -> MotionPredL1 [0] == 1){
			...
			if (!Nal -> non_diadic){
				int index_mv = RefPartitionL0 [0] . MvIndex;
				mvL0[0] = MV_CAL(mv_bas_l1[index_mv], Nal -> up_Width, Nal -> down_Width) + aio_pBlock -> MvdL1[0][0]; 
				mvL0[1] = MV_CAL(mv_bas_l1[index_mv + 1], Nal -> up_Height, Nal -> down_Height) + aio_pBlock -> MvdL1[0][1]; 
			}else{
	...
	if ( Nal -> PicToDisplay ){
		//L0 interpolation
		SampleInterpolationMain8x8( image_l0, image_Cb_l0, image_Cr_l0, MvCache_l0, 
			RefCache_l0, x, y,  PicWidthInPix, PicHeightInPix, RefPicListL0, vectors -> Ptr_interpol_8x8);

		//L0 interpolation
		SampleInterpolationMain8x8( image_l1, image_Cb_l1, image_Cr_l1, MvCache_l1, 
				 ...
				 short *mv_bas_l1, short *ref_bas_l0, short *ref_bas_l1, const STRUCT_PF *vectors, REFPART RefPartitionL0[16])
{
	...
	if ( Nal -> PicToDisplay ){
		//L0 interpolation
		SampleInterpolationMain8x8( image_l0, image_Cb_l0, image_Cr_l0, MvCache_l0, RefCache_l0, 
			x, y,  PicWidthInPix, PicHeightInPix, RefPicListL0, vectors -> Ptr_interpol_8x8);

		//L0 interpolation
		SampleInterpolationMain8x8( image_l1, image_Cb_l1, image_Cr_l1, MvCache_l1, RefCache_l1, 
