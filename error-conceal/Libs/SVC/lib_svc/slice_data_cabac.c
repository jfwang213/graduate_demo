			  ...
			  short ref_cabac_l0[], unsigned char *EndOfSlice, short *mvl0_io, short *refl0_io)
{
	...
	int mb_skip_flag = 0 ;
	int Last_Qp = slice -> slice_qp_delta ;
	int diff_qp = 0, ErrorDetection = 0;
	...
	do {
		...
		slice_table [iCurrMbAddr] = slice -> slice_num ;
		GetNeighbour(iCurrMbAddr, iMb_x, PicWidthInMbs, PicSizeInMbs, slice_table, picture_residu, 0);
		...
		mb_skip_flag = decode_cabac_mb_skip( cabac, cabac_state, picture_residu[iCurrMbAddr] . AvailMask, 
			slice_table, picture_residu, slice -> slice_type, iCurrMbAddr, PicWidthInMbs);
		if (mb_skip_flag){
			diff_qp = 0;
			...
			fill_caches_motion_vector_full_ref(slice, slice -> b_stride, slice -> b8_stride, 
				refl0_cache, mvl0_cache, slice_table , picture_residu, 
			...
			fill_caches_motion_vector( slice, PicWidthInMbs << 3, PicWidthInMbs << 1, ref_cache_l0, 
				mv_cache_l0, slice_table, &picture_residu[iCurrMbAddr], 
			...
			fill_caches_motion_vector_full_ref(slice, slice -> b_stride, slice -> b8_stride, refl0_cache, 
				mvl0_cache, slice_table , picture_residu, &mvl0_io[(iMb_x << 3) + iMb_y * (slice -> b_stride << 2)], 
				&refl0_io[(iMb_x << 2) + iMb_y * (slice -> b_stride << 1)], iMb_x, iMb_y);


			//Decoding process of the current macroblock
			ErrorDetection = macroblock_layer_P_cabac(&picture_residu[iCurrMbAddr], slice, block, non_zero_count_cache, slice_table, 
			  ...
			  short *mvl1_l0, short *mvl1_l1, short *refl1_l0, short *refl1_l1)
{
	...
	do {
		init_block(&block[iCurrMbAddr]);
		init_residu(&picture_residu[iCurrMbAddr]);
		picture_residu[iCurrMbAddr] . SliceNum = slice_table [iCurrMbAddr] = slice -> slice_num ;
		GetNeighbour(iCurrMbAddr, iMb_x, PicWidthInMbs, PicSizeInMbs, slice_table, picture_residu, 0);
		...
		if (mb_skip_flag){
			...
			fill_caches_motion_vector_B_full_ref(slice, slice -> b_stride, slice -> b8_stride, 
				refl0_cache, refl1_cache, mvl0_cache, mvl1_cache, slice_table , picture_residu, 
				&mvl0_io [(iMb_x << 3) + iMb_y * (slice -> b_stride << 2)], 
			...
			fill_caches_motion_vector_B( slice, PicWidthInMbs << 3, PicWidthInMbs << 1, 
				ref_cache_l0, ref_cache_l1, mv_cache_l0, mv_cache_l1, slice_table, &picture_residu[iCurrMbAddr], 
			...
			ErrorDetection = macroblock_layer_B_cabac(&picture_residu[iCurrMbAddr], slice, block, non_zero_count_cache, 
				slice_table, iCurrMbAddr, cabac, cabac_state, param_neigh, mv_cache_l0, mv_cache_l1, ref_cache_l0, 
						...
						unsigned char *end_of_slice,MMO* mmo) 
{
	if (!Nal -> NalToSkip){
		...
		if ( *end_of_slice == 0 ) {
			...
			mapUnitToSlice(sps, pps, pLayerMmo->mb_slice_group_map, slice -> slice_group_change_cycle, 
				pps->slice_group_id);
			InitUcharTab(sps -> PicSizeInMbs, pLayerMmo->mb_slice_table);
		}else{
		...
		if ( slice -> slice_type == SLICE_TYPE_B){
			Nal -> ErrorDetection = B_cabac (picture_residu, data, NalBytesInNalunit, sps, pps, Tab_block, slice, &pos, pLayerMmo->mb_slice_table, 
				pLayerMmo->mb_slice_group_map, mv_cabac_l0, mv_cabac_l1, ref_cabac_l0, ref_cabac_l1, end_of_slice, ai_pRefPicListL1, 
				Current_pic->mv_memory_l0,Current_pic->mv_memory_l1,
				Current_pic->ref_memory_l0,Current_pic->ref_memory_l1,
				ai_pRefPicListL1[0] . long_term, ai_pRefPicListL1[0] . slice_type, ai_pRefPicListL1[0].mv_memory_l0,
				ai_pRefPicListL1[0].mv_memory_l1,ai_pRefPicListL1[0].ref_memory_l0,ai_pRefPicListL1[0].ref_memory_l1);
			...
			Nal -> ErrorDetection = P_cabac(picture_residu, data, NalBytesInNalunit, sps, pps, Tab_block, slice, &pos,  
				pLayerMmo->mb_slice_table, pLayerMmo->mb_slice_group_map, mv_cabac_l0, ref_cabac_l0, end_of_slice, 
				Current_pic->mv_memory_l0,Current_pic->ref_memory_l0);
		}else {
			Nal -> ErrorDetection = slice_data_I_cabac (picture_residu, data, NalBytesInNalunit, sps, pps, 
				Tab_block, slice, &pos, pLayerMmo->mb_slice_table, pLayerMmo->mb_slice_group_map, end_of_slice );
		}
	}
	...
	//Error concealment
	AVCErrorConcealment(Nal, sps, slice, pLayerMmo->mb_slice_table, picture_residu,
		Current_pic->mv_memory_l0,Current_pic->mv_memory_l1,
		Current_pic->ref_memory_l0,Current_pic->ref_memory_l1);
#endif
}
