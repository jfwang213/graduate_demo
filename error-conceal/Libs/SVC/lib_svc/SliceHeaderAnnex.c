...
int SliceHeaderCut(unsigned char *data, SPS *sps, PPS *pps, NAL *Nal, int *position) 
{
	...
	SLICE Slice[1];
	MMO Mmo[1];
	LIST_MMO Current_pic[1], RefPicListL0[1], RefPicListL1[1];
	...
	frame_num = getNbits(data, position, pt_sps_id -> log2_max_frame_num );
	...
	if (Nal -> QualityId == 0){
		ReferenceListProcessing(data, position, Slice, pt_pps_id, pt_sps_id, LayerMmo, 
			Current_pic, RefPicListL0, RefPicListL1,Nal->UseRefBasePicFlag);
