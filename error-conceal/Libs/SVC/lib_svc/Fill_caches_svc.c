...
void fill_caches_svc ( const SLICE *ai_pstSlice,  const RESIDU *residu, const RESIDU *BaseResidu, const int ai_iEntropy_coding_mode_flag, 
					  unsigned char aio_tiNon_zero_count_cache [ RESTRICT ], const unsigned char ai_tiSlice_table [ ], const DATA *CurrBlock,  
					  ...
					  const int ai_iConstrained_intra_pred, const int level_prediction_flag)
{
	...
	short left_xy = 0;
	short pred;
    const short	iMb_xy = ai_iMb_x + ai_iMb_y * (short)(ai_pstSlice -> mb_stride);
	const short	ai_iMb_type_name = residu-> MbType;
	const unsigned char val_init = ai_iEntropy_coding_mode_flag && !(IS_I(ai_iMb_type_name)) ? 0 : 64 ;

	init_non_zero_count_cache(aio_tiNon_zero_count_cache, val_init);

	if ( ai_iMb_y > 0 ) {
		top_xy = iMb_xy - ai_pstSlice -> mb_stride;
		top_type = (ai_tiSlice_table [top_xy] == ai_pstSlice -> slice_num) ? residu [- ai_pstSlice -> mb_stride] . MbType : 0 ;
	}else {
		top_type = 0;
	}
	...
	if ( iMb_xy > 0 && ai_iMb_x != 0 )  {
		left_xy = iMb_xy - 1 ;
		left_type = ai_tiSlice_table [left_xy] == ai_pstSlice -> slice_num ? residu [-1] . MbType : 0 ;
	}else {
		left_type = 0;
	}

	FillNonZeroCount(residu, CurrBlock, ai_pstSlice -> mb_stride, top_type, left_type, aio_tiNon_zero_count_cache);


	//Updating the aio_tiIntra4x4_pred_mode_cache table from the current macroblock.
	if(IS_NxN(top_type))	{
		aio_tiIntra4x4_pred_mode_cache[4] = residu[- ai_pstSlice -> mb_stride] . Intra16x16DCLevel[10];
		aio_tiIntra4x4_pred_mode_cache[5] = residu[- ai_pstSlice -> mb_stride] . Intra16x16DCLevel[11];
		aio_tiIntra4x4_pred_mode_cache[6] = residu[- ai_pstSlice -> mb_stride] . Intra16x16DCLevel[14];
		aio_tiIntra4x4_pred_mode_cache[7] = residu[- ai_pstSlice -> mb_stride] . Intra16x16DCLevel[15];
    }
...
void fill_caches_motion_vector_full_ref(  const SLICE *ai_pstSlice, const int ai_iB_stride, const int ai_iB8_stride, 
										...
										const unsigned char ai_tiSlice_table [ ], const RESIDU CurrResidu [ ], short ai_tiMv[ RESTRICT ], 
										const short ai_tiRef[ ],  const int ai_iX, const int ai_iY)
{
    ...
    int    topright_type;
    int    left_type;
	const int iCurrMbAddr = ai_iX + ai_pstSlice -> mb_stride * ai_iY;
	const int top_xy = iCurrMbAddr - ai_pstSlice -> mb_stride ;
	...
	if ( top_xy >= 0 )     {
		top_type = ai_tiSlice_table [top_xy] == ai_pstSlice -> slice_num ? CurrResidu [top_xy] . MbType : 0 ;

		if ( topleft_xy >= 0 && ai_iX != 0 && ai_tiSlice_table [topleft_xy] == ai_pstSlice -> slice_num){
		...
		if ( topright_xy >= 0 && (ai_iX != ai_pstSlice -> mb_stride - 1) && ai_tiSlice_table [topright_xy] == ai_pstSlice -> slice_num){
    ...
    if ( left_xy >= 0 && ai_iX != 0 && ai_tiSlice_table [left_xy] == ai_pstSlice -> slice_num){
...
void fill_caches_full_ref(  const SLICE *ai_pstSlice, const int ai_iB_stride, const int ai_iB8_stride, short aio_tiRef_cache_l0 [ RESTRICT ], 
						  short aio_tiMv_cache_l0[ RESTRICT ][2], const unsigned char ai_tiSlice_table [ ], const RESIDU CurrResidu [ ], 
...
void fill_caches_motion_vector_B_full_ref(  SLICE *slice, int b_stride, int b8_stride, short ref_cache_l0 [], short ref_cache_l1 [], 
										  short  mv_cache_l0[][2], short  mv_cache_l1[][2], unsigned char slice_table [], const RESIDU CurrResidu [ ], 
										  short mv_l0[], short mv_l1[], short ref_l0[], short ref_l1[], int iCurrMbAddr, int ai_iX)
{
	...
	//Updating their availability according to the slice.
	if ( top_xy >= 0 )   {
		top_type = slice_table [top_xy] == slice -> slice_num ? CurrResidu [top_xy] . MbType : 0 ;
