...
void get_base_P_dyadic(short mv_cache [][2], short ref_cache [], unsigned char dpb_luma[], unsigned char dpb_Cb[], unsigned char dpb_Cr[], 
					   ...
					   LIST_MMO *RefPicListL0, short mv_l0[], short *ref_base, REFPART *RefPartition,  const NAL *Nal, const interpol_4x4 *interpol_4x4)
{
	...
	for ( Idx = 0 ; Idx < 16 ; Idx++ ) {
		...
		mv_cache [index][0] = MV_CAL(mv_l0[index_mv], Nal -> up_Width, Nal -> down_Width); 
		mv_cache [index][1] = MV_CAL(mv_l0[index_mv + 1],  Nal -> up_Height, Nal -> down_Height); 
	}


	if ( Nal -> PicToDisplay){
		//Recovery of  the prediction
		sample_interpolation(Y, U, V, mv_cache, ref_cache, x, y, PicWidthInPix, PicHeightInPix, RefPicListL0, interpol_4x4);	
	}
}
...
void get_base_P_non_dyadic(short mv_cache [][2], short ref_cache [],unsigned char image [], unsigned char image_Cb [], 
						   unsigned char image_Cr [], short x, short y, 
						   ...
						   const interpol_4x4 *interpol_4x4, REFPART *RefPartition)
{
	...
	if ( Nal -> PicToDisplay){
		//Recovery of  the prediction
		sample_interpolation( image, image_Cb, image_Cr, mv_cache, ref_cache, x , y, PicWidthInPix, PicHeightInPix, RefPicListL0, interpol_4x4);
	}
}
	...
	for ( Idx = 0 ; Idx < 4 ; Idx++ ) {
		for (Sub_Idx = 0; Sub_Idx < 4; Sub_Idx++){
			...
			if (ref_cache_l0 [index] >= 0){
				mv_cache_l0 [index][0] = MV_CAL(mv_l0[index_mv_l0], Nal -> up_Width, Nal -> down_Width); 
				mv_cache_l0 [index][1] = MV_CAL(mv_l0[index_mv_l0 + 1], Nal -> up_Height, Nal -> down_Height); 
			}
			//mbl check
			if (ref_cache_l1 [index] >= 0){
				mv_cache_l1 [index][0] = MV_CAL(mv_l1[index_mv_l1], Nal -> up_Width, Nal -> down_Width); 
				mv_cache_l1 [index][1] = MV_CAL(mv_l1[index_mv_l1 + 1], Nal -> up_Height, Nal -> down_Height); 
			}
		}
	}
...
int SvcSkipMotionVector(short mv_cache[][2], const short MvdLX[2], const short ref_cache[], short mvLX[2], int index8, int partwidth)
{
