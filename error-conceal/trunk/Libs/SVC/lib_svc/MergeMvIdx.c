	...
	for ( i = 0; i < 4; i++){
		...
		for ( j = 0; j < 4; j++){
			if ( MotionPred[(i & 2) + ((j & 2) >> 1)]){
				...
				mv_cache_l0[CacheIndex][0] = MV_CAL(mv_l0[MvIndex], Nal -> up_Width, Nal -> down_Width);
				mv_cache_l0[CacheIndex][1] = MV_CAL(mv_l0[MvIndex + 1], Nal -> up_Height, Nal -> down_Height);
			}
		}
	}
