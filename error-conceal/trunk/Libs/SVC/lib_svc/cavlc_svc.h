...
int macroblock_layer_in_scalable_extension(const NAL *Nal, const PPS *Pps, RESIDU *Current_residu, RESIDU *BaseResidu, 
										   ...
										   unsigned char aio_tiSlice_table [ ],  const short ai_iMb_x , const short ai_iMb_y, 
...
*/
static __inline int read_me_svc ( const unsigned char *tab,  int *pos, int mode, RESIDU *residu, const int mb_xy, const SLICE *slice  , const int x  )
{
	...
	if (slice -> notDefaultScanIdx){
		if(!val){
			...
			if((mb_xy - slice -> mb_stride) >= 0){
				return residu[- slice -> mb_stride] . Cbp;
			}		
...
static __inline int ReadMotionPredSVC( const unsigned char *ai_pcData, int *position, char *MotionPreLx, DATA *Macroblock, int Pred_Lx){
	...
	for ( mbPartIdx = 0 ; mbPartIdx < Macroblock -> NumMbPart ; mbPartIdx++ ) {
		if (Macroblock -> MbPartPredMode [mbPartIdx] != Pred_Lx) {
			MotionPreLx [mbPartIdx] = getNbits(ai_pcData, position, 1);
		}
	}
