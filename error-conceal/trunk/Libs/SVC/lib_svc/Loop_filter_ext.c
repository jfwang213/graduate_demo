...
*/
void Loop_filter_avc(SPS *Sps, PPS *pps, SLICE *slice, LIST_MMO *Current_pic, 
					 unsigned char slice_table[], DATA Tab_Block[], short aio_tiMv_l0[], short aio_tiMv_l1[], short aio_tiRef_l0[], 
	...
	if(slice -> disable_deblocking_filter_idc != 1){
		...
		for (iCurrMbAddr = 0 ; iCurrMbAddr < PicSizeInMbs ;iCurrMbAddr++) {
			...
			filter_mb_B(&Y [((iMb_x + (PicWidthInMbs << 4) * iMb_y) << 4)], &U [((iMb_x + (PicWidthInMbs << 3) * iMb_y) <<3)], 
				&V [((iMb_x + (PicWidthInMbs << 3) * iMb_y) <<3)], slice, pps, slice_table, mv_cache_l0, mv_cache_l1, 
...
*/
void Loop_filter_svc(const NAL *Nal, SPS *Sps, PPS *pps, SLICE *Slice, LIST_MMO *Current_pic, 
					 unsigned char slice_table[], short aio_tiMv_l0[], short aio_tiMv_l1[], short aio_tiRef_l0[], 
					 short aio_tiRef_l1[], RESIDU *curr_residu, unsigned char Y[], unsigned char U[], unsigned char V[])
{
	...
	if(Slice -> disable_deblocking_filter_idc != 1){
		...
		for (Pass = 0; Pass < NbPass; Pass ++){
			...
			for ( iCurrMbAddr = 0; iCurrMbAddr < PicSizeInMbs ; iCurrMbAddr++) {
				...
				SetLoopFilterQp(&curr_residu [iCurrMbAddr], Nal, iCurrMbAddr); 
				...
				filter_mb_svc(&Y [((iMb_x + (PicWidthInMbs << 4) * iMb_y) << 4)], 
					...
					&V [((iMb_x + (PicWidthInMbs << 3) * iMb_y) << 3)], Slice, pps, slice_table, 
					mv_cache_l0, mv_cache_l1, ref_cache_l0, ref_cache_l1, iCurrMbAddr,
					PicWidthInMbs, Current_pic -> ref_poc_l0, Current_pic -> ref_poc_l1, iMb_x, iMb_y, curr_residu, Nal, Mode);
						   ...
						   unsigned char Y[], unsigned char U[], unsigned char V[])
{
	...
	for ( ; iCurrMbAddr < PicSizeInMbs ;iCurrMbAddr++) {
		...
		if (IS_I_SVC(residu[iCurrMbAddr] . MbType)){
			filter_mb(&Y [((iMb_x + (PicWidthInMbs << 4) * iMb_y) << 4)], &U [((iMb_x + (PicWidthInMbs << 3) * iMb_y) << 3)], 
				&V [((iMb_x + (PicWidthInMbs << 3) * iMb_y) << 3)], pps, iCurrMbAddr, PicWidthInMbs, 
				residu[iCurrMbAddr] . Transform8x8, Nal -> LoopFilterSpatialScalability, DeblockingFilterIdc, iMb_x, iMb_y, residu);
		}
		iMb_x++;
		if( iMb_x == PicWiInMbs){
			iMb_x = 0;
