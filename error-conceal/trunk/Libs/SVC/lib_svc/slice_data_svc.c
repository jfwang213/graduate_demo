...
{	
	...
	do {
		...
		if( Slice -> slice_type  !=  EI ){
			...
			short mb_skip_run = read_ue(ai_pcData, position);
		...
		if ( moreDataFlag ) {
			...
			ErrorDetection = macroblock_layer_in_scalable_extension(Nal, Pps, &Current_residu[iCurrMbAddr], BaseResidu, ai_pcData, 
				position, Slice, aio_tstBlock, vlc, NonZeroCountCache, aio_tiSlice_table, iMb_x, iMb_y, 
...
{	
    ...
    volatile int moreDataFlag = 1 ;
	int diff_qp = 0;
	int MbSkipFlag, ErrorDetection = 0;
