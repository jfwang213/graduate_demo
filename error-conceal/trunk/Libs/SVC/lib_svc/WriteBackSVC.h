...
static __inline void write_back_P_skip_motion_full_ref_C( const int ai_iB_stride, const int ai_iB8_stride	, short MvdL0[ ], short ai_tiMv_cache[2], short ao_tiRef[ ])
{
...
static __inline void write_back_full_ref_C(const int ai_iB_stride	, const int ai_iB8_stride, short ai_tiMv[ ], short ai_tiMv_cache[ ][2], short ai_tiRef[ ], short aio_tiRef_cache[ ]){
	...
	for (y = 0; y < 4; y ++, ai_tiRef += ai_iB8_stride){
		aio_tiRef_cache[12 + 8 * y] = ai_tiRef[0];
		aio_tiRef_cache[13 + 8 * y] = ai_tiRef[1];
		aio_tiRef_cache[14 + 8 * y] = ai_tiRef[2];
		aio_tiRef_cache[15 + 8 * y] = ai_tiRef[3];
	}
}
...
static __inline void write_back_full_ref_8x8(const int ai_iB_stride, const int ai_iB8_stride, short ai_tiMv[ ], short ai_tiMv_cache[ ][2], short ai_tiRef[ ], short aio_tiRef_cache[ ]){
...
static __inline void write_back_motion_full_ref( const int ai_iB_stride, const int ai_iB8_stride, short MvdL0[ ], short ai_tiMv_cache[ ][2], short ao_tiRef[ ], const short ai_tiRef_cache[])
{
	...
	for(y = 0; y < 4; y++, ao_tiRef += ai_iB8_stride){
		ao_tiRef[0] = ai_tiRef_cache[12 + 8 * y];
		ao_tiRef[1] = ai_tiRef_cache[13 + 8 * y];
		ao_tiRef[2] = ai_tiRef_cache[14 + 8 * y];
		ao_tiRef[3] = ai_tiRef_cache[15 + 8 * y];
	}
}
