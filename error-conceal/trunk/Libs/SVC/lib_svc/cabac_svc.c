...
int CabacGetBaseModeFlag( CABACContext *c, unsigned char *state, RESIDU *CurrResidu,  unsigned char slice_table[], int mb_stride) {


	int AvailMask = CurrResidu -> AvailMask;
	int ctx = 2;

	if( (AvailMask & 0x01) && slice_table[0] == slice_table[- 1] && IS_BL(CurrResidu [-1] . MbType)){
		ctx--;
	}

	if( ((AvailMask >> 1) & 0x01) && slice_table[0] == slice_table[- mb_stride] && IS_BL(CurrResidu [- mb_stride] . MbType)){
		ctx--;
	}


	return get_cabac( c, &state[ctx] );
}
...
int CabacIMacroblockLayer(CABACContext *cabac, unsigned char *cabac_state, const NAL *Nal, SLICE *Slice, 
						   ...
						   const short iMb_y, int ConsIntraPredFlag, int DctAllowed, int *LastQP, int *MbQpDelta )
{
	...
	int BaseModeFlag = 0;
	int	InCropWindow = CurrentResidu -> InCropWindow;
	...
	return CabacIPartioning(Nal,BaseResidu,CurrentResidu, Slice, Tab_Block, CurrBlock, NonZeroCountCache, SliceTable, 
		iCurrMbAddr, cabac, cabac_state, Neigh, iMb_x, iMb_y, ConsIntraPredFlag, DctAllowed, LastQP, MbQpDelta);
}
...
int CabacPMacroblockLayer(CABACContext *cabac, unsigned char *cabac_state, const NAL *Nal, SLICE *Slice, 
						  RESIDU *TabResidu, RESIDU *BaseResidu, DATA  Tab_Block[], CABAC_NEIGH *Neigh, 
						  ...
						  int ConsIntraPredFlag, int DctAllowed, int *LastQP, int *MbQpDelta)
{
	...
	if (IS_I(CurrentResidu -> MbType)){
		return CabacIPartioning(Nal,BaseResidu,CurrentResidu, Slice, Tab_Block, CurrBlock, NonZeroCountCache, 
			SliceTable, iCurrMbAddr, cabac, cabac_state, Neigh, iMb_x, iMb_y, ConsIntraPredFlag, 
			DctAllowed, LastQP, MbQpDelta);
	} else {
...
int CabacBMacroblockLayer(CABACContext *cabac, unsigned char *cabac_state, const NAL *Nal, SLICE *Slice, RESIDU *TabResidu, 
						   ...
						   const short iMb_y, int ConsIntraPredFlag, int DctAllowed, int Direct8x8InferFlag,  int *LastQP, int *MbQpDelta)
{
