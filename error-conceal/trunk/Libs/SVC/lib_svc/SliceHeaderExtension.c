...
{
	if (Nal -> QualityId == 0){
		ReferenceListProcessing(data, position, Slice, pt_pps_id, pt_sps_id, LayerMmo, Current_pic, RefPicListL0, RefPicListL1,Nal->UseRefBasePicFlag);
		...
		if( Nal -> NalRefIdc != 0 ) {
			dec_ref_pic_marking(data, position, Nal -> IdrFlag * 5, pt_sps_id, LayerMmo,Current_pic);
							...
							LIST_MMO *Current_pic, MMO *Mmo, int EndOfSlice)
{
	...
	if ( !Nal -> no_inter_layer_pred_flag && Nal -> QualityId == 0){
		...
		if ( pt_sps_id -> InterLayerDeblockingFilterFlag){
			...
			if(Slice -> InterLayerDeblockingFilterIdc != 1 ) {
				pt_pps_id -> alpha_offset_upsapmpling = (unsigned char) (read_se(data, position) << 1); //int inter_layer_slice_alpha_c0_offset_div2 = 
				pt_pps_id -> beta_offset_upsapmpling = ((unsigned char) read_se(data, position) << 1);  //int inter_layer_slice_beta_offset_div2 = 
			}

		}else {
			Slice -> InterLayerDeblockingFilterIdc = 1;
			pt_pps_id -> alpha_offset_upsapmpling = 0;
			pt_pps_id -> beta_offset_upsapmpling = 0;
		}
		...
		int BaseLayerDqId = Nal -> layer_id_table [ Nal -> LayerId - 1];
		Nal -> BaseLayerId = Nal -> LayerId - 1;
		RealBaseLayer = GetLayerNumber(Nal, BaseLayerDqId);
	}else {
	...
	Nal->base_picture = Mmo->LayerMMO[RealBaseLayer].currentPic;
}
...
void SvcReadDefaultParameters(unsigned char *data, int *position, NAL *Nal, SLICE *Slice, SPS *pt_sps_id)
{
	...
	if ( !Nal -> no_inter_layer_pred_flag){ 
		...
		Slice->DefaultResidualPredictionFlag = 0;
		Slice->DefaultMotionPredictionFlag = 0;
