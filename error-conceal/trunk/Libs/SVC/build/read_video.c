...
access_unit* get_one_access_unit(SPS* all_sps,PPS* all_pps,decoder_context *pdecoder_context)
{
	...
	while (next_nal = get_one_nal(pdecoder_context))
	{
		char end_of_slice;
...
char read_video_init(int argc,char *argv[])
{
	...
	for (i = 0;i<argc-1;i++)
	{
		if (0 == strcmp("-h264",argv[i]))
		{
			fileName = argv[i+1];
		}
		else if (0 == strcmp("-network",argv[i]))
		{
			read_from_network = 1;
		}
	}
	if (NULL == fileName && !read_from_network)
	{
		printf("can't find fileName from parameters and not from network!\n");
		return 0;
	}
	init_access_unit(next_access_unit);
	if (read_from_network)
	{
		get_one_nal = get_one_nal_from_network;
		if (network_init())
		{
			return 1;
		}
	...
	{
		get_one_nal = get_next_nal_from_file;
