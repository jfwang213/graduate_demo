...
@param state State of the current slice.
@param Tab_Block Contains all parameters of the current macroblock.
@param slice_table Specifies in which aio_pstSlice belongs each macroblock of the picture.
@param macro Specifies the availability of the neighbouring macroblock.
@param mb_xy CUrrent address of the macroblock.
...
int decode_cabac_mb_type( CABACContext *c, unsigned char *state, RESIDU *CurrResidu, 
						 unsigned char slice_table[], int mb_xy, int slice_type, const int mb_stride)
{
	...
	if( slice_type == SLICE_TYPE_P ) {
		...
		if( (CurrResidu -> AvailMask & 0x01) && slice_table[mba_xy] == slice_table[mb_xy] && CurrResidu [-1] . MbType != B_Skip
			&& CurrResidu [-1] . MbType != B_direct  )
			ctx++;
		if( ((CurrResidu -> AvailMask >> 1) & 0x01) && slice_table[mbb_xy] == slice_table[mb_xy] && CurrResidu [- mb_stride] . MbType != B_Skip
...
*/
int macroblock_layer_B_cabac (RESIDU *picture_residu, SLICE *slice, DATA Tab_block[],  unsigned char non_zero_count_cache [], 
							   unsigned char slice_table [], int iCurrMbAddr, CABACContext *cabac, unsigned char *cabac_state, 
							   ...
							   int dct_allowed, int *last_QP, int *mb_qp_delta, int is_svc, short mvl0_cache[][2], 
...
{  
	...
	int mb_type = decode_cabac_mb_type(cabac, cabac_state, picture_residu, slice_table, iCurrMbAddr,  slice -> slice_type, slice -> mb_stride);
	...
	if (mb_type < 23){
		...
		return mb_cabac_B_partionning(picture_residu,  slice, Tab_block, non_zero_count_cache, slice_table, 
		...
		return mb_cabac_I_partionning(picture_residu, slice, Tab_block, block, non_zero_count_cache, slice_table, 
...
*/
int  macroblock_layer_I_cabac (RESIDU *picture_residu, SLICE *slice, DATA Tab_block[], unsigned char non_zero_count_cache [], 
							   unsigned char slice_table [], int iCurrMbAddr, CABACContext *cabac, unsigned char *cabac_state,
							   CABAC_NEIGH *neigh, short x, short y, int constrained_intra_pred_flag, int dct_allowed, int *last_QP, int *mb_qp_delta)
{  
...
*/
int macroblock_layer_P_cabac (RESIDU *picture_residu, SLICE *slice, DATA Tab_block[], unsigned char non_zero_count_cache [], 
							   unsigned char slice_table [], int iCurrMbAddr, CABACContext *cabac, unsigned char *cabac_state, 
							   CABAC_NEIGH *neigh, short mv_cache_l0[][2], short ref_cache_l0[], short x, short y, 
							   int constrained_intra_pred_flag, int dct_allowed, int *last_QP, int *mb_qp_delta, 
...
{  
	...
	if (mb_type < 5){
		...
		return mb_cabac_P_partionning(picture_residu,  slice, Tab_block, block,  non_zero_count_cache, slice_table, 
