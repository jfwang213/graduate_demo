...
int macroblock_B_partitionning (const PPS *Pps, RESIDU *Current_residu, const unsigned char *ai_pcData, 
								int *aio_piPosition, SLICE *aio_pstSlice, DATA *aio_pstBlock, const VLC_TABLES * Vlc, 
								...
								int long_term, int slice_type, int is_svc, int *last_QP, int iCurrMbAddr, 
								...
								short *mvl1_l0, short *mvl1_l1, short *refl1_l0, short *refl1_l1)
{
	...
	//Updating the aio_pstSlice table in order to save in which aio_pstSlice each macroblock belong to
	...
	if (aio_pstBlock -> NumMbPart == 4 ) {
		if(sub_mb_pred_B(&dct_allowed, ai_pcData, aio_piPosition, aio_pstSlice, Current_residu, 
		...
		if(mb_pred_B(ai_pcData, aio_piPosition, aio_pstSlice, aio_pstBlock, Current_residu, 
			mv_cache_l0, mv_cache_l1, ref_cache_l0, ref_cache_l1, mvl1_l0, mvl1_l1, 
			refl1_l0, refl1_l1, direct_8x8_inference_flag, long_term, slice_type, is_svc)){
				return 1;
		}
	}else {
		motion_vector_B_skip(mv_cache_l0, mv_cache_l1, ref_cache_l0, ref_cache_l1, 
			mvl1_l0, mvl1_l1, refl1_l0, refl1_l1, aio_pstSlice, 
...
*/
int macroblock_layer_B (const PPS *Pps, RESIDU *picture_residu,  const unsigned char *ai_pcData,  int *aio_piPosition, 
						SLICE *aio_pstSlice, DATA *aio_tstBlock, const VLC_TABLES * Vlc, unsigned char *aio_tiNon_zero_count_cache, 
						unsigned char  *aio_pstSlice_table, const short ai_iMb_x, const short ai_iMb_y, int direct_8x8_inference_flag, 
						int long_term, int slice_type, int is_svc, int *last_QP, short mv_cache_l0[][2], short mv_cache_l1[][2], 
						short *ref_cache_l0, short *ref_cache_l1, short *mvl1_l0, short *mvl1_l1, short *refl1_l0, short *refl1_l1)
{  


	const int iCurrMbAddr = ai_iMb_x + ai_iMb_y * (short)(aio_pstSlice -> mb_stride);
	...
	//According to the aio_pstSlice type and the macroblock type, the parameters are adjusted
	if (mb_type < 23)	{
		...
		return macroblock_B_partitionning(Pps, picture_residu, ai_pcData, aio_piPosition, aio_pstSlice, 
			aio_pstBlock, Vlc, aio_tiNon_zero_count_cache, aio_pstSlice_table, ai_iMb_x, ai_iMb_y, 
	...
	else
	{
		...
		return macroblock_I_partitionning(Pps, picture_residu, ai_pcData, aio_piPosition, 
			aio_pstSlice, aio_pstBlock, Vlc, aio_tiNon_zero_count_cache, aio_pstSlice_table, 
