...
@param slice_table Specifies in which slice belongs each macroblock of the picture.
...
*/
void slice_data_B ( const SPS *ai_stSps, const PPS *ai_stPps, SLICE *ai_stSlice, const unsigned char  ai_tSlice_table [], 
				   ...
				   unsigned char aio_tImage [], unsigned char aio_tImage_Cb [], unsigned char aio_tImage_Cr [], const W_TABLES *quantif)
{
	...
	for ( ; iCurrMbAddr < PicSizeInMbs ;) {
		...
		unsigned char *Y = &aio_tImage[offset_L + ((iMb_x + ( iMb_y * PicWidthInPix)) << 4)];
		unsigned char *U = &aio_tImage_Cb[offset_C + ((iMb_x + ( iMb_y * PicWidthInPix >> 1)) << 3)];
		unsigned char *V = &aio_tImage_Cr[offset_C + ((iMb_x + ( iMb_y * PicWidthInPix >> 1)) << 3)];
		...
		if (  IS_I(picture_residu [iCurrMbAddr] . MbType)) {
			...
			decode_MB_I(Y, U, V	, ai_stPps, &picture_residu[iCurrMbAddr], PicWidthInMbs, quantif, main_vectors -> baseline_vectors);
			...
			if( picture_residu [iCurrMbAddr] . Mode == 0){
				...
				sample_interpolation_main(image_l0, image_l1, image_Cb_l0, image_Cb_l1, image_Cr_l0, image_Cr_l1, 
				...
				SampleInterpolationMain8x8( image_l0, image_Cb_l0, image_Cr_l0, 
				...
				SampleInterpolationMain8x8( image_l1, image_Cb_l1, image_Cr_l1,
			...
			decode_P_chroma(U, V, ai_stPps, &picture_residu[iCurrMbAddr], PicWidthInPix >> 1, quantif);
...
void non_IDR_main ( SPS *ai_stSps, PPS *ai_stPps , SLICE *ai_stSlice,  unsigned char ai_tSlice_table [], 
				   ...
				   short aio_tref_l1[], unsigned char aio_tDpb_luma[], unsigned char aio_tDpb_Cb[],unsigned char aio_tDpb_Cr[])
{
	...
	if (ai_stSps -> PicSizeInMbs){ 
		slice_data_B (ai_stSps,ai_stPps, ai_stSlice, ai_tSlice_table,  picture_residu, main_vector, ai_pRefPicListL0, 
			ai_pRefPicListL1, Current_pic->mv_memory_l0, Current_pic->mv_memory_l1, 
			Current_pic->ref_memory_l0, Current_pic->ref_memory_l1, 
			aio_tDpb_luma, aio_tDpb_Cb, aio_tDpb_Cr	, Current_pic->picture_memory_y, 
			Current_pic->picture_memory_u, Current_pic->picture_memory_v, quantif);
	}
}
