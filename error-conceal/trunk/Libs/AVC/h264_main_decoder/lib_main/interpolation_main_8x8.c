									   ...
									   unsigned char refPicLXCr[] , short xFracl, short yFracl, short PicWidthSamples)
{
	...
	for(j = 0; j < 4; j++)	{
		for(i = 0; i < 4; i++){
			...
			image_Cb[(j << 2) + i] = (cst2*A + cst3*B + cst4*C + cst1*D + 32) >> 6;
			image_Cr[(j << 2) + i] = (cst2*E + cst3*F + cst4*G + cst1*H + 32) >> 6;
			*ptr_img_Cb++;
			*ptr_img_Cr++;
		}
...
@param dpb_luma Table of the reference decoded picture buffer.
@param dpb_Cb Table of the reference decoded picture buffer.
@param dpb_Cr Table of the reference decoded picture buffer.
...
*/
void interpolation_main_8x8(unsigned char dpb_luma[], unsigned char dpb_Cb[],unsigned char dpb_Cr[], 
							...
							short mvL0[2], short PicWidthInPix, const interpol_8x8 * interpol)
{
	...
	interpol[xFracl + (yFracl << 2)](image, dpb_luma, PicWidthInPix, 8);


	//Chrominance interpolation
	xFracl = (short)  mvL0[0]&7;
	yFracl = (short)  mvL0[1]&7;
	chroma_SampleInterpolationMain8x8(image_Cb,image_Cr, dpb_Cb, dpb_Cr, xFracl, yFracl, PicWidthInPix>>1);
...
@param interpol Function pointer of 8x8 interpoaltion.
*/
void SampleInterpolationMain8x8(unsigned char Luma_l0[] , unsigned char Chroma_Cb_l0[], unsigned char Chroma_Cr_l0[], 
								...
								short PicHeightInPix , const LIST_MMO * RefPicListL0, const interpol_8x8 *interpol)
{
	...
	if(ref_cache_l0[12] >= 0){
		const int offset_L = (RefPicListL0[ref_cache_l0[12]].pic_width+32)*16+16;
		const int offset_C = (RefPicListL0[ref_cache_l0[12]].pic_width+32)*4+8;
		unsigned char* luam_memory = RefPicListL0[ref_cache_l0[12]].picture_memory_y+offset_L;
		unsigned char* cb_memory = RefPicListL0[ref_cache_l0[12]].picture_memory_u+offset_C;
		unsigned char* cr_memory = RefPicListL0[ref_cache_l0[12]].picture_memory_v+offset_C;	
		int xIntl = CLIP3(-11, PicWidthInPix - 27, x + (mvL0[12][0] >> 2));
		int yIntl = CLIP3(-11, PicHeightInPix - 27, y + (mvL0[12][1] >> 2));

		interpolation_main_8x8(luam_memory + xIntl + yIntl * PicWidthInPix, 
			cb_memory + (xIntl >> 1) + (yIntl >> 1) * (PicWidthInPix >> 1), 
			cr_memory + (xIntl >> 1) + (yIntl >> 1) * (PicWidthInPix >> 1), 
			Luma_l0, Chroma_Cb_l0, Chroma_Cr_l0, mvL0[12], PicWidthInPix, interpol);
	}


	if(ref_cache_l0[14] >= 0){
		const int offset_L = (RefPicListL0[ref_cache_l0[14]].pic_width+32)*16+16;
		const int offset_C = (RefPicListL0[ref_cache_l0[14]].pic_width+32)*4+8;
		unsigned char* luam_memory = RefPicListL0[ref_cache_l0[14]].picture_memory_y+offset_L;
		unsigned char* cb_memory = RefPicListL0[ref_cache_l0[14]].picture_memory_u+offset_C;
		unsigned char* cr_memory = RefPicListL0[ref_cache_l0[14]].picture_memory_v+offset_C;		
	...
	if(ref_cache_l0[28] >= 0){
		const int offset_L = (RefPicListL0[ref_cache_l0[28]].pic_width+32)*16+16;
		const int offset_C = (RefPicListL0[ref_cache_l0[28]].pic_width+32)*4+8;
		unsigned char* luam_memory = RefPicListL0[ref_cache_l0[28]].picture_memory_y+offset_L;
		unsigned char* cb_memory = RefPicListL0[ref_cache_l0[28]].picture_memory_u+offset_C;
		unsigned char* cr_memory = RefPicListL0[ref_cache_l0[28]].picture_memory_v+offset_C;	
	...
	if(ref_cache_l0[30] >= 0){
		const int offset_L = (RefPicListL0[ref_cache_l0[30]].pic_width+32)*16+16;
		const int offset_C = (RefPicListL0[ref_cache_l0[30]].pic_width+32)*4+8;
		unsigned char* luam_memory = RefPicListL0[ref_cache_l0[30]].picture_memory_y+offset_L;
		unsigned char* cb_memory = RefPicListL0[ref_cache_l0[30]].picture_memory_u+offset_C;
		unsigned char* cr_memory = RefPicListL0[ref_cache_l0[30]].picture_memory_v+offset_C;	
