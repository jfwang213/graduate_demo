...
void slice_data_B_cavlc ( RESIDU *picture_residu, const unsigned char *ai_pcData, const int ai_piNalBytesInNalunit, 
						 ...
						 int *aio_piPosition,  const VLC_TABLES *Vlc, unsigned char aio_tiSlice_table [],  
						 const unsigned char ai_tiMbToSliceGroupMap [], unsigned char *EndOfSlice, LIST_MMO *RefListl1, 
						 ...
						 short *mvl1_l0, short *mvl1_l1, short *refl1_l0, short *refl1_l1)
{
    ...
    do 
    {
		...
		short mb_skip_run = read_ue(ai_pcData, aio_piPosition);
		if (mb_skip_run){
			for ( i = 0 ; i < mb_skip_run && iCurrMbAddr < PicSizeInMbs; i++ ) {		
				...
				SkipB(Slice, Sps, picture_residu, aio_tstBlock, aio_tiSlice_table, iMb_x, iMb_y, Last_Qp, mv_cache_l0, ref_cache_l0, 
		...
		if ( moreDataFlag ) {		
			...
			fill_caches_motion_vector_B(Slice, Slice -> b_stride, Slice -> b8_stride, 
				...
				aio_tiSlice_table , &picture_residu [iCurrMbAddr], 
            ...
            ErrorDetection = macroblock_layer_B(Pps, &picture_residu[iCurrMbAddr], ai_pcData, aio_piPosition, 
				Slice, aio_tstBlock, Vlc, aio_tiNon_zero_count_cache, aio_tiSlice_table, iMb_x, iMb_y, 
			...
			NonZeroCountManagement(aio_tiNon_zero_count_cache, iMb_x, iMb_y, PicWidthInMbs, 
