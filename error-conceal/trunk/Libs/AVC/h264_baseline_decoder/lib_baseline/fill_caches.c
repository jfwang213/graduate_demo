...
void FillNonZeroCount( const RESIDU CurrResidu[], const DATA *CurrBlock, const int MbStride,const int top_type, const int  left_type
					  , unsigned char *NonZeroCountCache){


	//Updating the aio_tiNon_zero_count_cache table from the current macroblock.
	if ( top_type ){
