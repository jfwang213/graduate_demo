				...
				while (1)
				{
					...
					if(reordering_of_pic_nums_idc < 3){
						if(reordering_of_pic_nums_idc < 2){
							...
							if (pred < 0)
							{
								pred += sps->max_pic_num;
							}
							else
							{
								pred %= sps->max_pic_num;
							}
							for(i = 0; i<LayerMmo->dep_mmo->ShortRefCount;i++)
							{
								if(LayerMmo->dep_mmo->ShortRef[i].pic_num == pred){
									*ref =  LayerMmo->dep_mmo->ShortRef[i];
									break;
								}
							}
							...
							{
								if (ref_list[i].long_term && ref_list[i].pic_id == long_term_pic_num)
								{
									*ref = ref_list[i];
									break;
								}
						...
						{
							if (ref_list[i].pic_id == ref->pic_id)
							{
								break;
							}
						}
...
*/
void GetMMCOOperationCode(SPS *sps,LAYER_MMO* layerMmo, char* layerIndex, unsigned char *data, 
						  int *Position,MMCO* Mmco)
{
	int i;
	for(i = 0; i < MAX_MMCO_COUNT; i++) {
		...
		if (  Mmco[i] . opcode == 1 ||  Mmco[i] . opcode == 3 ) {
			//Mmco[i] . short_frame_num = (current_pic -> pic_num - read_ue(data, Position) - 1) & ((1 << (sps -> log2_max_frame_num)) - 1) ;// difference_of_pic_nums_minus1 =
			Mmco[i] . short_frame_num = read_ue(data, Position);
		}
		if (  Mmco[i] . opcode == 2 ) {
						  ...
						  SPS *sps, LAYER_MMO *LayerMmo,LIST_MMO* current_pic)
{
	...
	if ( nal_unit_type == 5 ) {	
		...
		if(long_term_reference_flag  == -1)
			...
			dep_mmo -> index = 1;
			dep_mmo -> Mmco[0] . opcode = 6;
			dep_mmo -> Mmco[0] . long_term_frame_idx = 0;
		}
	} else {
		if ( getNbits(data, Position, 1) ) {
			GetMMCOOperationCode(sps,LayerMmo, &(dep_mmo->index), data, Position,
				dep_mmo->Mmco);
		}else{
			dep_mmo -> index = 0;
		}
...
void FrameHeaderData(unsigned char *data, int *Position, int NalRefIdc, int IdrFlag, int LayerId, 
					 int EndOfSlice, LIST_MMO *Current_pic, SPS *sps_id, PPS *pps_id, 
					 SLICE* current_slice)
{

	if ( !sps_id -> frame_mbs_only_flag ) {
		current_slice -> field_pic_flag = getNbits(data, Position, 1);
		if ( current_slice -> field_pic_flag ) {
			current_slice->bottom_field_flag = getNbits(data, Position, 1);
		}
	}
	else
	{
		current_slice->field_pic_flag = 0;
	}

	if(!current_slice -> field_pic_flag){
		Current_pic -> pic_num = current_slice -> frame_num;
		...
		Current_pic -> pic_num = 2 * current_slice -> frame_num + 1;
		sps_id -> max_pic_num = 1 << (sps_id -> log2_max_frame_num + 1);
	}
	if ( IdrFlag == 1) {
		current_slice->idr_pic_id = read_ue(data, Position);//idr_pic_id =
		if (current_slice -> slice_type == SLICE_TYPE_I){
	...
	if ( sps_id -> pic_order_cnt_type == 0 ) {
		current_slice -> pic_order_cnt_lsb = getNbits(data, Position, sps_id -> log2_max_pic_order_cnt_lsb ); 
		if ( pps_id -> pic_order_present_flag && !current_slice -> field_pic_flag ) 
			current_slice -> delta_pic_order_cnt_bottom = read_se(data, Position);
	}

	if ( sps_id -> pic_order_cnt_type == 1 && !sps_id -> delta_pic_order_always_zero_flag ) {
		current_slice -> delta_pic_order_cnt [0] = read_se(data, Position); 
		if ( pps_id -> pic_order_present_flag && !current_slice -> field_pic_flag ) 
			current_slice -> delta_pic_order_cnt [1] = read_se(data, Position);
	}
...
void ReferenceListProcessing(unsigned char *data, int *Position, SLICE *Slice, 
							 ...
							 LIST_MMO *Current_pic, LIST_MMO *RefPicListL0, LIST_MMO *RefPicListL1,int bUserBaselayer)
{
	if ( Slice -> slice_type == SLICE_TYPE_B ) {
	...
	if ( Slice -> slice_type == SLICE_TYPE_P
		...
		|| Slice -> slice_type == SLICE_TYPE_SP ) {
			...
			if ( num_ref_idx_active_override_flag ) {
				...
				{
					Slice -> num_RefIdxL1_active_minus1 = 0;
				}
			} else {
	...
	LayerMmo->dep_mmo -> RefCountL0 = Slice -> num_RefIdxL0_active_minus1 + 1;
	LayerMmo->dep_mmo -> RefCountL1 = Slice -> num_RefIdxL1_active_minus1 + 1;


	if (Slice -> slice_type != SLICE_TYPE_I){
		fill_default_ref_list(LayerMmo->dep_mmo, RefPicListL0, RefPicListL1, Current_pic, Slice -> slice_type,bUserBaselayer);
	}
...
void QpAndDeblockProcessing(unsigned char *data, int *Position, SLICE *Slice, PPS *pps_id, SPS *sps_id)
{
	...
	pps_id -> alpha_offset = 0;
	pps_id -> beta_offset = 0;

	if ( pps_id -> deblocking_filter_control_present_flag ) {
		Slice -> disable_deblocking_filter_idc = read_ue(data, Position);
		if ( Slice -> disable_deblocking_filter_idc != 1 ) {
			pps_id -> alpha_offset = (read_se(data, Position) << 1);
			pps_id -> beta_offset = (read_se(data, Position) << 1);
		}
	}
...
void slice_header ( unsigned char *data, int *nal_unit_type, int *nal_ref_idc, 
				   ...
				   int *ImgToDisplay, int *xsize, int *ysize, int *AddressPic, int *Crop,int dq_id_display)

{
	if( mmo -> MemoryAllocation){
		...
		*sps_id = sps[seq_parameter_set_id];
		Slice -> frame_num = getNbits(data, Position, sps_id -> log2_max_frame_num );
		...
		*EndOfSlice = PrevEndOfSlice && Slice -> frame_num == LastFrameNum && FirstMbInSlice != Slice -> first_mb_in_slice? 1: 0;
		...
		FrameHeaderData(data, Position, *nal_ref_idc, *nal_unit_type == 5, 0, *EndOfSlice, 
			Current_pic, sps_id, pps_id, Slice);


		if ( !*EndOfSlice){
			pic_order_process(sps_id, *nal_unit_type, *nal_ref_idc, mmo, LayerMmo, Current_pic, 
				RefPicListL0, RefPicListL1,Slice,dq_id_display);
			allocate_picture_memory(Current_pic);
		}


		//Get the number of reference frames and initialize the reference lists.
		ReferenceListProcessing(data, Position, Slice, pps_id, sps_id, LayerMmo, Current_pic, RefPicListL0, RefPicListL1,0);
		...
		if ( *nal_ref_idc != 0 ) {
			dec_ref_pic_marking(data, Position, *nal_unit_type, sps_id, LayerMmo,Current_pic);
		}
...
@param aio_pstSlice_table Specifies in which Slice belongs each macroblock of the picture.
...
void slice_layer_without_partitioning_rbsp ( unsigned char *ai_pcData, const int ai_piNalBytesInNalunit, 
											...
											unsigned char aio_pstSlice_table[], DATA aio_tstBlock[], RESIDU *picture_residu, 
											unsigned char *EndOfSlice, short *mvl0_io, short *refl0_io) 
{
	...
	if(Slice -> slice_type == SLICE_TYPE_P){
		slice_data_P_cavlc (picture_residu, ai_pcData, ai_piNalBytesInNalunit, 
			Sps,Pps, aio_tstBlock, Slice, &pos, Vlc, aio_pstSlice_table, 
