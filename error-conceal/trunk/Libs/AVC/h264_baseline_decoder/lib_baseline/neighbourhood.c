...
@param PicWidthInPix Width of the sample to interpolate.
@param aio_pstSlice_table Table which contains for each macroblock the number of the slice they belong to.
...
*/
void GetNeighbour(short Addr, short iAddr_x, const int PicWidthInMbs, const int PicSizeInMbs, 
				  const unsigned char aio_pstSlice_table [], RESIDU CurrResidu [], const int constained_pred_flag)
{
	...
	if ( constained_pred_flag){
		Mask =   IS_I( CurrResidu [mbAddrA] . MbType) && (iAddr_x != 0)   && NeighbourAvail(mbAddrA, PicSizeInMbs, aio_pstSlice_table[mbAddrA], aio_pstSlice_table[Addr]);
		Mask += (IS_I( CurrResidu [mbAddrB] . MbType) &&					 NeighbourAvail(mbAddrB, PicSizeInMbs, aio_pstSlice_table[mbAddrB], aio_pstSlice_table[Addr])) << 1;
		Mask += (IS_I( CurrResidu [mbAddrC] . MbType) && (iAddr_xp1 != 0) && NeighbourAvail(mbAddrC, PicSizeInMbs, aio_pstSlice_table[mbAddrC], aio_pstSlice_table[Addr])) << 2;
		Mask += (IS_I( CurrResidu [mbAddrD] . MbType) && (iAddr_x != 0)   && NeighbourAvail(mbAddrD, PicSizeInMbs, aio_pstSlice_table[mbAddrD], aio_pstSlice_table[Addr])) << 3;

		//For SVC
		//Working only with on slice per frame
		Mask += (IS_I( CurrResidu [mbAddrE] . MbType) && (iAddr_xp1 != 0) && NeighbourAvail(mbAddrE, PicSizeInMbs, aio_pstSlice_table[mbAddrE], aio_pstSlice_table[Addr])) << 4;
		Mask += (IS_I( CurrResidu [mbAddrF] . MbType) && (iAddr_x != 0)   && NeighbourAvail(mbAddrF, PicSizeInMbs, aio_pstSlice_table[mbAddrF], aio_pstSlice_table[Addr])) << 5;
		Mask += (IS_I( CurrResidu [mbAddrG] . MbType)					  && NeighbourAvail(mbAddrG, PicSizeInMbs, aio_pstSlice_table[mbAddrG], aio_pstSlice_table[Addr])) << 6;
		Mask += (IS_I( CurrResidu [mbAddrH] . MbType) && (iAddr_xp1 != 0) && NeighbourAvail(mbAddrH, PicSizeInMbs, aio_pstSlice_table[mbAddrH], aio_pstSlice_table[Addr])) << 7;
	}else{
		Mask =  ((iAddr_x != 0)   && NeighbourAvail(mbAddrA, PicSizeInMbs, aio_pstSlice_table[mbAddrA], aio_pstSlice_table[Addr]));
		Mask +=	(				     NeighbourAvail(mbAddrB, PicSizeInMbs, aio_pstSlice_table[mbAddrB], aio_pstSlice_table[Addr])) << 1;
		Mask += ((iAddr_xp1 != 0) && NeighbourAvail(mbAddrC, PicSizeInMbs, aio_pstSlice_table[mbAddrC], aio_pstSlice_table[Addr])) << 2;
		Mask += ((iAddr_x != 0)   && NeighbourAvail(mbAddrD, PicSizeInMbs, aio_pstSlice_table[mbAddrD], aio_pstSlice_table[Addr])) << 3;
		Mask += ((iAddr_xp1 != 0) && NeighbourAvail(mbAddrE, PicSizeInMbs, aio_pstSlice_table[mbAddrE], aio_pstSlice_table[Addr])) << 4;
		Mask += ((iAddr_x != 0)   && NeighbourAvail(mbAddrF, PicSizeInMbs, aio_pstSlice_table[mbAddrF], aio_pstSlice_table[Addr])) << 5;
		Mask +=	(			         NeighbourAvail(mbAddrG, PicSizeInMbs, aio_pstSlice_table[mbAddrG], aio_pstSlice_table[Addr])) << 6;
		Mask += ((iAddr_xp1 != 0) && NeighbourAvail(mbAddrH, PicSizeInMbs, aio_pstSlice_table[mbAddrH], aio_pstSlice_table[Addr])) << 7;
	}
