...
static __inline void luma_sample_interp_1_2_2pass(short tmp_res [RESTRICT], unsigned char ptr_imag[RESTRICT], const short stride)
{
	...
	{
		ptr_imag[0] = (ptr_imag[0] + CLIP255_16((tmp_res[0] + 16) >> 5) + 1) >> 1;
		ptr_imag[1] = (ptr_imag[1] + CLIP255_16((tmp_res[1] + 16) >> 5) + 1) >> 1;
		ptr_imag[2] = (ptr_imag[2] + CLIP255_16((tmp_res[2] + 16) >> 5) + 1) >> 1;
		ptr_imag[3] = (ptr_imag[3] + CLIP255_16((tmp_res[3] + 16) >> 5) + 1) >> 1;
	}
}
