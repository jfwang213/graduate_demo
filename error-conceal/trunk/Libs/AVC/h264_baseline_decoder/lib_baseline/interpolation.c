...
void Interpolate(int Ref, short Currentx, short Currenty, short *mvLX, const short OutStride, 
				 ...
				 unsigned char *Y, unsigned char*U, unsigned char *V,const interpol_4x4 *interpol)
{

	const int offset_l = 16+16*(RefPicListL0[Ref].pic_width+32); 
	const int offset_c = 8+4*(RefPicListL0[Ref].pic_width+32);
	unsigned char* ref_luma = RefPicListL0[Ref].picture_memory_y+ offset_l;
	unsigned char* ref_cb = RefPicListL0[Ref].picture_memory_u + offset_c;
	unsigned char* ref_cr = RefPicListL0[Ref].picture_memory_v + offset_c;
	...
	unsigned char *ptr_dpb_Cb = ref_cb + (xIntl >> 1) + ( yIntl >> 1 ) * (PicWidthInPix >> 1);
	unsigned char *ptr_dpb_Cr = ref_cr+ (xIntl >> 1) + ( yIntl >> 1 ) * (PicWidthInPix >> 1);

	//Luminance interpolation 
	short xFracl = currentMvx&3;
	short yFracl = currentMvy&3;
	interpol[xFracl + (yFracl << 2)](Y, ref_luma + yIntl * PicWidthInPix + xIntl, PicWidthInPix, OutStride);
...
*/
void sample_interpolation( unsigned char image [], unsigned char image_Cb [], unsigned char image_Cr [], 
