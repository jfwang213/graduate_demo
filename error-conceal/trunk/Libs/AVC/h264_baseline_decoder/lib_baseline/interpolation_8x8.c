...
*/
void interpolation_8x8(unsigned char dpb_luma[], unsigned char dpb_Cb[],unsigned char dpb_Cr[], 
					   ...
					   short mvL0[2],  short PicWidthInPix, const interpol_8x8 *interpol)
{
		...
		interpol[xFracl + (yFracl << 2)](image, dpb_luma, PicWidthInPix, PicWidthInPix);


		//Chrominance interpolation
		xFracl = mvL0[0] & 7;
		yFracl = mvL0[1] & 7;
		chroma_SampleInterpolation8x8(image_Cb, image_Cr, dpb_Cb, dpb_Cr, xFracl, yFracl, PicWidthInPix >> 1);
}
...
@param interpol Function pointer of 8x8 interpoaltion.
*/
void SampleInterpolation8x8(unsigned char Luma_l0[] , unsigned char Chroma_Cb_l0[], unsigned char Chroma_Cr_l0[], 
							...
							short PicHeightInPix , const LIST_MMO *RefPicListL0, const interpol_8x8 *interpol)
{
	...
	const int offset_l = 16+16*(RefPicListL0[ref_cache_l0[12]].pic_width+32);
	const int offset_c = 8+4*(RefPicListL0[ref_cache_l0[12]].pic_width+32);
	//Recovery of  the prediction
	char* luma_memory_address = RefPicListL0[ref_cache_l0[12]].picture_memory_y+offset_l;
	char* cb_memory_address = RefPicListL0[ref_cache_l0[12]].picture_memory_u+offset_c;
	char* cr_memory_address = RefPicListL0[ref_cache_l0[12]].picture_memory_v+offset_c;
	int xIntl = CLIP3(-11, PicWidthInPix - 27, x + (mvL0[12][0] >> 2));
	int yIntl = CLIP3(-11, PicHeightInPix - 27, y + (mvL0[12][1] >> 2));
		
	interpolation_8x8(luma_memory_address + xIntl + yIntl * PicWidthInPix, 
		cb_memory_address + (xIntl >> 1) + (yIntl >> 1) * (PicWidthInPix >> 1), 
		cr_memory_address + (xIntl >> 1) + (yIntl >> 1) * (PicWidthInPix >> 1), 
		Luma_l0, Chroma_Cb_l0, Chroma_Cr_l0, mvL0[12], PicWidthInPix, interpol);


	luma_memory_address = RefPicListL0[ref_cache_l0[14]].picture_memory_y+offset_l;
	cb_memory_address = RefPicListL0[ref_cache_l0[14]].picture_memory_u+offset_c;
	cr_memory_address = RefPicListL0[ref_cache_l0[14]].picture_memory_v+offset_c;	
	...
	luma_memory_address = RefPicListL0[ref_cache_l0[28]].picture_memory_y+offset_l;
	cb_memory_address = RefPicListL0[ref_cache_l0[28]].picture_memory_u+offset_c;
	cr_memory_address = RefPicListL0[ref_cache_l0[28]].picture_memory_v+offset_c;	
	...
	luma_memory_address = RefPicListL0[ref_cache_l0[30]].picture_memory_y+offset_l;
	cb_memory_address = RefPicListL0[ref_cache_l0[30]].picture_memory_u+offset_c;
	cr_memory_address = RefPicListL0[ref_cache_l0[30]].picture_memory_v+offset_c;	
