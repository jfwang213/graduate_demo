...
@param aio_tiNon_zero_count_cache Specifies the coeff_token of each blocks 4x4 of a macroblock.
*/
void residual_block_cavlc_16_C( short *ao_piCoeffLevel, const int ai_iMaxNumCoeff, const unsigned char *RESTRICT ai_poData, int *position, 
							   const int ai_iN,	const VLC_TABLES *vlc, unsigned char aio_tiNon_zero_count_cache [ RESTRICT ])
{
	...
	int tmp = pred_non_zero_count(n, aio_tiNon_zero_count_cache);
	...
	*position += tmp ;
	...
	if(ai_iN != LUMA_DC_BLOCK_INDEX){
		aio_tiNon_zero_count_cache [SCAN8(ai_iN)] = coeff_token_TotalCoeff;
	}else{
		aio_tiNon_zero_count_cache [0] = coeff_token_TotalCoeff;
	}
...
void residual_block_cavlc_64( short *   ao_piCoeffLevel, const int scantable[], const unsigned char * RESTRICT ai_poData, int * position, 
							 const int n, const VLC_TABLES *vlc, unsigned char aio_tiNon_zero_count_cache [ RESTRICT ])
{
	int             i ;
	int				coeff_token_TotalCoeff;
	int				coeff_token_TrailingOnes;  
	...
	*position += tmp ;
	...
	coeff_token_TotalCoeff = TotalCoeff(coeff_token);
	aio_tiNon_zero_count_cache [SCAN8(n)] = coeff_token_TotalCoeff;
...
*/
void residual (const unsigned char *ai_pcData, int *position, RESIDU *Residu, const VLC_TABLES *Vlc,  unsigned char aio_tiNon_zero_count_cache [ ]){
	...
	if ( Residu -> MbType == INTRA_16x16 ){
		residual_block_cavlc_16(Residu -> Intra16x16DCLevel, 16, ai_pcData, position, 26, Vlc , aio_tiNon_zero_count_cache);

		if( Residu -> Cbp & 15 ) {
			for( i = 0; i < 16; i++ ) {
				residual_block_cavlc_16(&Residu -> LumaLevel [i * 16], 15, ai_pcData, position, i, Vlc, aio_tiNon_zero_count_cache);
			}
		}else {

			//Updating the non_zero_count_cache table for the neighbourhood
			aio_tiNon_zero_count_cache [12]	= aio_tiNon_zero_count_cache [13] = aio_tiNon_zero_count_cache [14] = aio_tiNon_zero_count_cache [15] = 
				aio_tiNon_zero_count_cache [20] = aio_tiNon_zero_count_cache [21] = aio_tiNon_zero_count_cache [22] = aio_tiNon_zero_count_cache [23] = 
				aio_tiNon_zero_count_cache [28] = aio_tiNon_zero_count_cache [29] = aio_tiNon_zero_count_cache [30] = aio_tiNon_zero_count_cache [31] = 
				aio_tiNon_zero_count_cache [36] = aio_tiNon_zero_count_cache [37] = aio_tiNon_zero_count_cache [38] = aio_tiNon_zero_count_cache [39] = 0;		
		}
	}
		...
		for ( i8x8 = 0 ; i8x8 < 4 ; i8x8++ ) {	
			if ( Residu -> Cbp & 1 << i8x8 ) {
				...
				for ( i4x4 = 0 ; i4x4 < 4 ; i4x4++ ) {
					...
					residual_block_cavlc_64(&Residu -> LumaLevel [i8x8 * 64], zigzag_scan8x8_cavlc + 16 * i4x4, ai_pcData, position, index, Vlc, aio_tiNon_zero_count_cache);

				}
				ind = SCAN8(i8x8 << 2);
				aio_tiNon_zero_count_cache[ind] |= aio_tiNon_zero_count_cache[ind + 1] | 
					aio_tiNon_zero_count_cache[ind + 8] |  aio_tiNon_zero_count_cache[ind + 9];
			} 
			else {
				const int index = SCAN8(i8x8 << 2);
				//Updating the non_zero_count_cache table for the neighbourhood
				aio_tiNon_zero_count_cache [index] = aio_tiNon_zero_count_cache [index + 1] = 
					aio_tiNon_zero_count_cache [index + 8] = aio_tiNon_zero_count_cache [index + 9] = 0 ;
		...
		for ( i8x8 = 0 ; i8x8 < 4 ; i8x8++ ){			
			if ( Residu -> Cbp & 1 << i8x8 ) {
				...
				for ( i4x4 = 0 ; i4x4 < 4 ; i4x4++ ) {
					...
					//Recovery of the Intra 4x4 luminance AC
					residual_block_cavlc_16(&Residu -> LumaLevel [index << 4], 16, ai_pcData, position, index, Vlc , aio_tiNon_zero_count_cache);
				}
			} 
			else 
			{
	...
	if ( Residu -> Cbp & 0x20 )     {
		...
		for ( i4x4 = 0 ; i4x4 < 4 ; i4x4++ ) {
			...
			residual_block_cavlc_16(&Residu -> ChromaACLevel_Cb [i4x4 * 16], 15, ai_pcData, position, index, Vlc , aio_tiNon_zero_count_cache);
		}

		//Chrominance Cr
		for ( i4x4 = 0 ; i4x4 < 4 ; i4x4++ ) {
			const int   index = 16 + 4 + i4x4 ;            
			residual_block_cavlc_16(&Residu -> ChromaACLevel_Cr [i4x4 << 4], 15, ai_pcData, position, index, Vlc, aio_tiNon_zero_count_cache);
		}
	} 
	else 
	{
		//Updating the non_zero_count_cache table for the neighbourhood
		aio_tiNon_zero_count_cache [9 + 0] = aio_tiNon_zero_count_cache [9 + 1] = 
			aio_tiNon_zero_count_cache [9 + 8] = aio_tiNon_zero_count_cache [9 + 9] = 
			aio_tiNon_zero_count_cache [33 + 0] = aio_tiNon_zero_count_cache [33 + 1] = 
			aio_tiNon_zero_count_cache [33 + 8] = aio_tiNon_zero_count_cache [33 + 9] = 0 ;
	}
}
