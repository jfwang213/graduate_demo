...
*/
void init_mmo( int num_of_layers, MMO *mmo,decoder_context* pdecoder_context){
	int i, j;
		
	memset(mmo, 0, sizeof(MMO));
	memset(mmo -> Prev, -1, sizeof(LIST_MMO));
	mmo -> low_delay = 1;	
	mmo->layer_id_table = pdecoder_context->layer_id_table;
	for ( j = 0; j < num_of_layers; j++){
		mmo->LayerMMO[j].dep_mmo = NULL;
		mmo->LayerMMO[j].mb_slice_group_map = NULL;
		mmo->LayerMMO[j].mb_slice_table = NULL;
		mmo->LayerMMO[j].currentPic->bBasePic = 0;
		init_list_wrap(mmo->LayerMMO[j].eclist);
	}
	for (j = 0;j<NUM_DEP_LAYERS;j++)
	{
		mmo->DepMMO[j].frame_num_gap_check = 0;
		for (i = 0;i<MAX_REF;i++)
		{
			mmo->DepMMO[j].LongRef[i].poc = MIN_POC;
		}
	}
}
...
void init_slice ( SLICE *aio_pstSlice ){
	...
	memset (aio_pstSlice,'\0',sizeof(SLICE));
}
...
{
	aio_pstSlice -> mb_xy = 0;
	aio_pstSlice -> slice_num = 0;
}
